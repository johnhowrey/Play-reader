...
export default function AnnotationMarker({
  ...
  // In read-only mode with no annotations, don't render anything
  if (readOnly && !hasAnnotations) return null;
    ...
    <div className="relative inline-flex" ref={panelRef}>
      ...
      {open && (
        ...
        >
          ...
          {/* New annotation form — hidden in read-only mode */}
          {!readOnly && (
          ...
          {readOnly && (
            <p className="text-xs text-muted text-center italic pt-1">
              Read-only — upgrade to add notes
            </p>
          )}
